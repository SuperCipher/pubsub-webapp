...
export const fetchTweets = async (filters = []) => {
    ...
    const tweets = await program.value.account.event.all(filters);
...
export const paginateTweets = (filters = [], perPage = 6, onNewPage = () => {}) => {
    ...
    const prefetchCb = async () => {
        ...
        const tweetClient = program.value.account.event
    ...
    const pageCb = async (page, paginatedPublicKeys) => {
        const tweets = await program.value.account.event.fetchMultiple(paginatedPublicKeys)
